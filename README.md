...
- [X] Explain what a token is used for.
    - Tokens are used for authenticating a user to authorize access to often protected materials. A good example of a token would be something that uses single sign-on (SSO) as a method of logging into an account. It helps to protect sensitive information from being accessed by unauthorized persons. 
...
- [X] What steps can you take in your web apps to keep your data secure?
    ...
    - Create a private route that uses a token in order to resrtic access to authorized persons only
    - Keep your apps up to date

- [X] Describe how web servers work.
    - Servers store, process and deliver requested data or webages to the client using HTTP (hypertext transfer protocol) or other protocols (such as TCP/IP or OSI). The server and the client use communcation protocols to proces incoming network requests.
